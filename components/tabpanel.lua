...
function TabPanel:draw()
	dxDrawRectangle(self.x, self.y, self.w, self.h, tocolor(0,0,0, 0))	

	local activeTab = self.activeTab
	if activeTab then
		activeTab:setVisible(true)
		activeTab.button.color = tocolor(204, 204, 204, 255)
...
function TabPanel:setActiveTab(tab)
	self.activeTab.button.color = tocolor(39, 174, 95, 255)
	self.activeTab = tab
end
